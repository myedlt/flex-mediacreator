package classes.transform 
{
	...
	public class TransformTool extends UIComponent 
	{
		...
		// With getter/setters
		private var _target : IMediaObject;
		...
		private var _scale9GridEnabled : Boolean = false;
		...
		public var outOfBorderline : Function;

		
		private var _originalWidth : Number;		private var _originalHeight : Number;
		private var isInteractioned : Boolean;
		private var oldToolMatrix : Matrix;
		 ...
		 * @return
		 */	
		[Bindable("controlTransformTool")]
		public function get targetW() : Number
		{
			return _target.transform.pixelBounds.width;
		} 

		public function set targetW(w : Number) : void
		{
			oldToolMatrix = currentTargetMatrix;
			scaleXInteraction( w );
			apply( );
			dispatchEvent( new Event( TransformTool.CONTROL_TRANSFORM_TOOL ) );
			matrixChange( );
		} 
		 ...
		 */
		[Bindable("controlTransformTool")]
		public function get targetH() : Number
		{
			return _target.transform.pixelBounds.height;
		} 

		public function set targetH(h : Number) : void
		{
			oldToolMatrix = currentTargetMatrix;
			scaleYInteraction( h );
			apply( );
		 ...
		 * @return 
		 */		
		public function get targetOldW() : Number
		{
		 ...
		 * @return 
		 */		
		public function get targetOldH() : Number
		{
		...
		{
			return _target.transform.pixelBounds.x;
		} 
		...
		{
			return _target.transform.pixelBounds.y;
		} 
		...
		{
			oldToolMatrix = currentTargetMatrix;
			moveInteraction( Infinity, y );
			apply( );
			dispatchEvent( new Event( TransformTool.CONTROL_TRANSFORM_TOOL ) );
			matrixChange( );
		} 

		public function updateMatrixAndView(v : Matrix) : void
		{
			toolMatrix = v;
			dispatchEvent( new Event( TransformTool.CONTROL_TRANSFORM_TOOL ) );
		}

		private function matrixChange() : void
		{
			dispatchEvent( new TransformToolEvent( TransformToolEvent.TARGET_MATRIX_CHANGE, _target.uid,oldToolMatrix, currentTargetMatrix ) );
		}
		[Bindable("controlTransformTool")]
		...
		public function setControlDisable() : void
		{
			_customCursorsEnabled = false;
			_cursorsEnabled = false;
			_customControlsEnabled = false;
		 ...
		 */
		public function get target() : IMediaObject 
		{
			return _target;
		}

		public function set target(v : IMediaObject) : void 
		{
			var d : DisplayObject = v as DisplayObject;
			...
			}else
			{
				...
				_target = v;
				_originalWidth = _target.width;
				_originalHeight = _target.height;
				//set scale9Grid
				updateMatrix( );
				setNewRegistation( );
				updateControlsVisible( );
				if(Object( _target ).isLock)
				{
					setControlDisable( );
				}
		...
		public function get currentTargetMatrix() : Matrix
		{
			if(_target != null)	return _target.transform.matrix.clone( );
			return null;
		}
		...
		public function set scale9GridEnabled(b : Boolean) : void 
		{			
			_scale9GridEnabled = b;
		}

		public function get scale9GridEnabled() : Boolean 
		{
			return _scale9GridEnabled;
		}
		...
		public function startInteractionHandler(event : MouseEvent) : void 
		{
		...
		private function interactionHandler(event : MouseEvent) : void 
		{
			...
			//第一次出发变更，记录最初就矩阵值
			if(!isInteractioned)	oldToolMatrix = currentTargetMatrix;
		...
		{
			//trace( "endInteractionHandler" );
			if(isInteractioned)	matrixChange( );
		 ...
		 */
		public function moveInteraction(x : Number = Infinity,y : Number = Infinity) : void 
		{
			if(outOfBorderline != null)
			{
				if(outOfBorderline( ))
				{
					return;
		 ...
		 */
		public function registrationInteraction(x : Number = Infinity,y : Number = Infinity) : void 
		{
			if(x != Infinity && y != Infinity)
			{
				_registration.x = x;
			...
			//trace(_registration.x,_registration.y);
			innerRegistration = toolInvertedMatrix.transformPoint( _registration );
		 ...
		 */
		public function rotationInteraction(degree : Number = Infinity) : void 
		{
		...
		public function scaleXInteraction(w : Number = Infinity) : void 
		{
            ...
            else
			{
				...
				//trace( _target.rotation );
				//trace( _toolMatrix.a, _toolMatrix.b );
				//trace(distortH.x,distortH.y,_toolMatrix.a,_toolMatrix.b)
			}
			completeInteraction( true );
		}
		 ...
		 */
		public function skewXInteraction(x : Number = Infinity) : void 
		{
		 ...
		 */
		public function skewYInteraction(y : Number = Infinity) : void 
		{
		...
		private function updateBounds() : void 
		{
			if (_target) 
			{
				...
				targetBounds = _target.getBounds( DisplayObject(_target) );
		...
		private function raiseTarget() : void 
		{
			...
			_target.parent.setChildIndex( DisplayObject(_target), index );
...
import classes.transform.TransformToolControl;
import classes.transform.TransformToolCursor;
...
class TransformToolMoveShape extends TransformToolInternalControl 
{
	...
	override public function draw(event : Event = null) : void 
	{
		...
		}else
		{
			...
			hitArea = null;
			currTarget = DisplayObject(_transformTool.target);
